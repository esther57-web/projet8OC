...
const Activity = () => {
  ...
  return (
    <section className={`${theme === "dark" ? "dark" :  "light"} overflow-hidden xs:py-8 lg:py-12 xs:px-8 flex`}>
              ...
              <div className="flex xs:gap-16 max-w-full pr-4 md:gap-32 lg:gap-48 xl:gap-72">
                {activitiesSlider.map((activity, index)=> (
                    <div key={index} className="flex items-center gap-2">
                        ...
                        <h3 className="xs:text-xl md:text-[1.2rem] lg:text-[1.5rem] whitespace-nowrap">{activity.name}</h3>
                    </div>
                ))}
              </div>
            </div> 
