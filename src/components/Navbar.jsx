...
const Navbar = () => {
  ...
  return (
    <header className={`flex z-50 fixed top-0 items-center xs:px-2 xs:py-2 w-full border-b border-cyan-900 lg:justify-between sm:justify-normal sm:py-0 sm:px-6 lg:px-12 ${theme === "dark"? "dark" : "light"}`}> 
            ...
            <div className='flex md:mr-4 sm:relative'>
                <nav className={`${isNavbarVisible ? "xs:block xs:absolute xs:left-1/2 xs:top-8 xs:transform -translate-x-1/2 xs:py-2 xs:border-b xs:border-cyan-900" : "xs:hidden"} ${theme === "dark" ? "dark" : "light"} sm:flex w-full sm:relative sm:top-auto`}>
                    ...
                    {navCategories.map((nav, index) => (
                        <a key={index} href={nav.link} className={`${theme === "dark" ? "sm:hover:bg-pp-gradient" : "sm:hover:bg-slate-600 sm:hover:text-white"} flex xs:leading-relaxed sm:py-5 lg:py-6 sm:px-2 lg:px-4 justify-center items-center cursor-pointer transition duration-300 xs:text-xs lg:uppercase lg:text-lg`}>{nav.name}</a>
                    ))}
                </nav>
