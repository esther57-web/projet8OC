...
const Navbar = () => {
    ...
    console.log(theme)
  return (
    <nav className={`flex sticky items-center xs:px-2 xs:py-2 w-full border-b border-cyan-900 lg:justify-between sm:justify-normal sm:py-0 lg:px-4 ${theme === "dark"? "dark" : "light"}`}> 
        ...
        <div className="flex xs:flex-row-reverse xs:w-full items-center relative sm:flex-row sm:justify-center lg:justify-end"> 
            <div className='flex md:mr-4 sm:relative'>
                <div className={`${isNavbarVisible ? "xs:block xs:absolute xs:left-1/2 xs:top-8 xs:transform -translate-x-1/2 xs:py-2 xs:border-b xs:border-cyan-900" : "xs:hidden"} ${theme === "dark" ? "dark" : "light"} sm:flex w-full sm:relative sm:top-auto`}>
